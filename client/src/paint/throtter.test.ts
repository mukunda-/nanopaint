...
describe("Throttler", () => {
   ...
   test("SingleThrottler request limiting", () => {
      ...
      for (let i = 0; i < burst; i++) {
         ////////////////////////////////////////////////////////////////////////////////
         // The `allow` function will return the delay until the next request is allowed.
         // Zero is returned when the request is allowed now. When getting a zero
         // response from allow, it is treated as a successful request (and it uses up
         // quota).
         expect(throttler.allow()).toBe(0);
      }

      ////////////////////////////////////////////////////////////////////
      // Once the quota is exceeded, it regenerates at `period` intervals.
      expect(throttler.allow()).toBe(period);

      jest.advanceTimersByTime(100);
      expect(throttler.allow()).toBe(period - 100);
      
      jest.advanceTimersByTime(period - 100 + 1);
      expect(throttler.allow()).toBe(0);
      expect(throttler.allow()).toBe(period - 1);
      ...
      expect(throttler.allow()).toBe(period - 50 * burst);
